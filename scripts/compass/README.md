...
| Script | Where | What |
|---|---|---|
| `gate_cpu.sh` | CPU container | The CPU test tier, run per task: every test file outside `tests/plugin/` and `cpu_gate_exclude.txt`, no driver **as a batch**: run alone, a tier file can still reach `rocminfo` (`tests/test_postprocess_width.py` does, through `from aiter import …`; measured on node 18, 2026-09-23T19:45Z). **Must be green.** Also exits 98 when the diff is in the blind spot below and the GPU tier has not run on *this* tree. Refuses a caller-supplied `-r`. |
