...
| Script | Where | What |
|---|---|---|
| `gate_cpu.sh` | CPU container | The CPU test tier, run per task: every test file outside `tests/plugin/` and `cpu_gate_exclude.txt`, no driver. **Must be green.** Also exits 98 when the diff is in the blind spot below and the GPU tier has not run on *this* tree. Refuses a caller-supplied `-r`. |
...
measured counter-example, and 1 and 3 name the commit they were measured at; the
fourth, collection, changed no path at `236abfd9a` and is kept as a forward guard:
