...
| O29 | the capture pool's reserved half, which `capture_reserved_parts` had to be *given* | existing artifacts only -- **done**, CPU, no new run | **closed at three widths, and now derived rather than supplied.** v1 was refuted because it modelled a captured LM head above one rank. The source guards that statement: `self.logits_in_graph = self.world_size == 1 and not is_tbo` (`model_runner.py:4104`) gates `graph_logits = self.model.compute_logits(...)` at `model_runner.py:4298`, inside `with torch.cuda.graph(...)`. The allocations that statement makes are identified **by site, not by size**: `head_sites.py` selects on the `capture_cudagraph` frame standing at line 4298 and finds exactly 6, one per captured bucket, every one raised by `torch_gemm@tuned_gemm.py:450` under `ParallelLMHead.forward`, none freed inside the window -- and finds **no** vocabulary-width request anywhere else in the capture body, so the removal takes that family and nothing else. `width_transform_v2.py` drops them at width > 1 and inserts no all-gather, because at width > 1 the head is not in the graph at all. Frozen as `frozen/pool_candidate_v2.json`, md5 `743b00da792156e43384f55eec19e7aa`, **before** any record was opened; v1 and its refutation are preserved unchanged. The one-way diagnostic (`pool_diagnostic_v2.py`, X27 opened only there) reads **+0.000% at every width on both sides**: reserved 127 926 272 / 106 954 752 / 85 983 232 predicted and measured at TP=1/2/4, and pool-side allocated 31 288 320 / 0 / 0 likewise. The TP=1 row is the source window and is not evidence; the TP=2 and TP=4 rows are held-out X27 and were never read while the candidate was built. The mechanism is now shipped: `AllocatorPool` and `allocator_pool_bytes` replay a request/free *stream* through the c10 rules with no observed segment size or address as input, and `capture_reserved_parts(pool_stream=...)` returns `pool_reserved_derived: True` where it used to return False -- the recorded-figure path is unchanged and still reports False. `capture_pool_predict.py` runs the whole chain from a history and reproduces the frozen manifest through the shipped model. What this does **not** close: the transform needs a TP=1 capture history *for that model and bucket list* -- the execution order is the one input no config transformation produces, and it is assumed unchanged across widths; nothing is inserted for the collectives a TP>1 forward runs; request sizes that do not resolve to `bs x config width x 2 B` are carried unchanged; the split/best-fit/coalescing rules stay empirical; the 1 024 B `CAPTURE_FIXED_PINNED` discrepancy stands. Provenance: v1's residuals had been seen when v2 was written, and no residual, record value or difference was an input to any rule in it |
| O30 | the whole prediction at the registered cell, per term rather than per closure: what a TP=2/TP=4 sizing still cannot be founded on | existing artifacts only -- **done**, CPU, no new run | **the capture and activation halves carry; `non_torch` above one rank does not, and it alone blocks the block count.** `freeze_e2e.py` writes `frozen/e2e_prediction.json` (md5 `21f45f87b878d3f90fbe369d5c0d2821`) before any record is opened, from checkpoint headers, `config.json`, the TP=1 capture history, the S27 TP=1 calibration and one allocation set read out of shipped library source. It calls `derived_readings` at width 1 only, on purpose: above it, `non_torch` and `load_residue` have no admissible basis, and the `DEFAULT_NON_TORCH` / `DEFAULT_LOAD_RESIDUE` entries at widths 2 and 4 are a **different engine** (the 0.6B) -- not standalone-component evidence for this one -- so they are refused rather than carried, as is `_at_width`'s silent carry-forward of the width-1 number. The one-way per-term evaluator (`e2e_diagnostic.py`, md5-gated, records opened only there, each record's `config` asserted equal to the registered cell first) reads, TP=1 / TP=2 / TP=4: weights **+1.490% / -0.248% / -3.552%** (checkpoint headers; the TP=4 row is the known 2-D-shards-1-D-replicates approximation), activation **-0.035% / -0.030% / -6.334%** (inductor instant against `peak_torch - current_torch`), `cudagraph_overhead` **-0.035% / -0.030% / -6.334%** (it is a function of the activation term, which is why it moves with it), and capture reserved **+0.000% / +0.000% / +0.000%** with 0 / 6 / 6 LM-head allocations dropped. Four distinct gates, four distinct definitions; the capture-pool correction was never expected to move the activation row and did not. `peak_torch` closes to +1.463% at TP=1 and is above it an **incomplete partial estimate** (-3.736% / -9.881%) and not a proven bound: it omits the terms that have no basis, so its sign is not guaranteed and it must never be used to reject a configuration. `kv blocks` is -0.684% at TP=1 and **refused** at TP=2/TP=4 -- `blocks_from_readings` needs all five readings and `free` follows `non_torch`. New, and the one term with a source-derived candidate: `CustomAllreduce.__init__` / `_init_ipc` / `IPCBufferPool.create` in the shipped aiter wheel allocate, on gfx942 and flat in world size, `rank_data` 8 MiB and an `input` pool of `max_size` (1 GiB default) **through** the torch allocator, and a `meta` pool of `meta_size() + 2 x max_size` via `hipDeviceMallocUncached`, **outside** it. That gives a load-residue candidate of 1 082 130 432 B (class SRC-AITER, conditional on expandable segments off and `AITER_CUSTOM_AR_RAW_INPUT_POOL` unset), which the diagnostic reads at **-51.786% / -53.439%** against `weights_torch - parameter_bytes` -- about 1.08 GiB short, and *not* flat: the measured term grows 79 694 080 B from TP=2 to TP=4. So the source read is a lower bound and something else allocates through the allocator at init; it is carried as a candidate, not accepted. Requested, minimal: a standalone component run -- no model, no engine -- on 2 and then 4 GPUs, recording device-wide used and allocator-allocated after (a) a bare context, (b) process-group init, (c) `CustomAllreduce` construction, and reporting `PYTORCH_HIP_ALLOC_CONF` and `AITER_CUSTOM_AR_RAW_INPUT_POOL` as set, since those move 1 GiB between the two terms. The (a)->(c) device-wide delta is TOTAL memory, **not** `non_torch`: `non_torch` is that used figure minus `torch.cuda.memory_reserved()`, the engine's own definition at `model_runner.py:1648`; the allocator delta settles the candidate |
| O31 | `non_torch` and `load_residue` above TP=1, which O30 left with no admissible basis and one half-sized source candidate | GPU, 4 devices, standalone -- **done**, probe `T27/1`, node18 GPU0-3, 2026-09-12, lease granted and released | **closed for both terms, as a delta with a measured control.** The probe runs the engine's own entrypoint with **no model**: `aiter.init_dist_env(world, backend="nccl", pp/dp/pcp/dcp=1)` exactly as `model_runner._setup_device_and_distributed` calls it in the non-PP branch, with `GroupCoordinator.__init__` and `CustomAllreduce.__init__` wrapped so every construction brackets itself with a reading. Class T27/1: no checkpoint, no KV cache, no target-engine number in, and TP=1 runs first as the control. **The count was measured, not assumed, and the source read was wrong by a factor of two**: seven `GroupCoordinator`s are constructed and **two** of them (`tp` and `ep`) have `world_size > 1`, so each rank builds **two** `CustomAllreduce` instances, each with its own `rank_data` and pools. The torch-allocator delta across `init_dist_env` is **2 164 260 864 B** -- `2 x (8 MiB + 1 GiB)` -- identical on every rank at TP=2 and TP=4 and flat in width, which is exactly twice O30's SRC-AITER candidate of 1 082 130 432 and is why that candidate read ~52% short. The candidate's *mechanism* was right; its instance count was not. `non_torch` per the engine's own definition (`max(used - reserved, 0)`, `model_runner.py:1648` -- **reserved**, not allocated) comes out **0 at TP=1** (at `world_size == 1` `GroupCoordinator` builds no device communicator at all, `parallel_state.py:570-573`), **5 676 990 464 B at TP=2 on both ranks exactly**, and at TP=4 **5 827 985 408 - 5 962 203 136 B**, a spread of exactly 134 217 728 B (128 MiB) across ranks. Phase-resolved: RCCL/pynccl bring-up +799 014 912 (TP=2) / +924 844 032 (TP=4), a second bring-up step +578 813 952 / +704 643 072, and **+2 149 580 800 per `CustomAllreduce`** -- the uncached `meta` pool, `_ops_meta_size() + 2 x max_size`, outside the allocator as the source read said. Allocation is **not lazy and not size-gated**: a first `all_reduce`, a second, and a 32 MiB one each moved nothing outside the allocator, so the whole cost is paid at init. The composition is wired into the real path in `atom/compass/core/memory_topology.py` (`compose_calibration` -> the width-keyed calibration `derived_readings` already consumes, so nothing downstream learns about this module) as **`term(width) = S27 term at TP=1 + (standalone(width) - standalone(TP=1))`**. The TP=1 control measuring zero for both terms is what makes that a delta rather than a substitution: width 1 composes to the S27 calibration *identically*, by construction, so no phase is double-counted and the engine term keeps context, driver and model-path costs the probe never runs. **The absolute-vs-delta choice was fixed by which phase each measurement brackets (`context_primed` -> `after_init_dist_env`), not by comparing against a target residual**; no record was opened until after the freeze. An unmeasured width is **refused** (`UnmeasuredWidth`) rather than answered from a narrower one -- the `_at_width` carry-forward is the failure this module exists to prevent -- and so is a mismatched allocator/pool environment, since under `expandable_segments` or `AITER_CUSTOM_AR_RAW_INPUT_POOL` the 2 GiB of input pools move from `load_residue` into `non_torch` and both numbers are wrong. `non_torch` is per-rank, so the point value is the **rank maximum** (under-reserving it inflates the KV cache and kills the run at steady state; over-reserving costs blocks) with the range published beside it, not averaged. Frozen first as `agent_scratch/memval/capture_replay/frozen/e2e_prediction_v2.json` md5 `512657c3ff781c06c74d2d5c7e0176d3`, with v1 (md5 `21f45f87b878d3f90fbe369d5c0d2821`) preserved unedited as the record of what had no basis before the probe. The md5-gated one-way diagnostic then reads, held out at both widths: `load_residue` **-2.906% at TP=2 and -6.236% at TP=4**, against O30's -51.786% / -53.439%; `non_torch` **-7.990% / -8.663%**, which is the same sign and roughly the same size as the **-2.817% at TP=1 where the calibration is exact by construction** -- the record's `non_torch` is device-wide and includes whatever else sat on the device, so part of that gap is not the model's to explain; and `kv_blocks` **+0.495% / +0.899%**, where both widths were previously refused outright. **No coefficient was changed as a result of any of those numbers.** TP=2/TP=4 remain class X27 and diagnostic; acceptance is still the e2e cc-traces gates. Two things are explicitly carried rather than closed: the residual `non_torch` gap above what the device-wide reading can account for, and the teardown observation -- after `destroy_dist_env` + `empty_cache`, `memory_reserved` is 0 but device-used is still ~8.4/8.7 GiB -- which is **probe-scoped and deferred**, since the probe's own census and its `get_tp_group()` local can keep a `CustomAllreduce` reachable past the teardown that was meant to drop the last reference. Nothing in the deltas depends on it: they are bracketed long before any teardown |
