...
and `plan_pools` are ATOM's and stay ATOM's (`03` D14).
...
no `__int__`, and prints as a per-term table. `03` D16 is the reason: a summed
non-KV check once read +13.8% while holding a 25% error in one of its terms,
and an object that can be printed only as a total reproduces that by design.
