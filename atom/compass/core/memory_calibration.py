...
What is deliberately *not* here is `non_torch`. It is `(total - free) -
reserved`, which is device-wide: it charges this configuration for whatever the
neighbours hold. The TP=1 source record reads 32 MiB above what the same
configuration reads on an exclusively-held device, so calibrating it here would
freeze a neighbour's allocation into the model. It needs an exclusive-device
source run of its own before it can be a constant.
"""
...
class SourceCalibration:
    ...
    def classify(self, term: str, config: Mapping) -> str:
        """`residual` if `config` is the calibration's own run, else `validation`.
        ...
        if entry is None:
            return "underived"
        return "residual" if entry.run.matches(config) else "validation"
...
def for_model(model: str) -> Optional[SourceCalibration]:
