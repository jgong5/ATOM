...
| **KV *transfer* (PD disagg)** | **SIMULATED** | No RDMA, no real bytes on a fabric. A simulated connector registered in ATOM's existing factory charges `latency + bytes/bandwidth` from the machine spec. `allocate_kv_cache` refuses, by name, any `kv_connector` other than `compass`, including a `kv_transfer_config` with no `kv_connector` key, which reads as `moriio`. With no `kv_transfer_config` at all it builds no connector and refuses nothing. `01` D6 |
