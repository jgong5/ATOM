...
- **Per task — the CPU tier, re-measured by P0.1 and superseding the counts above.**
  ...
  **129 of this tree's 188 test files, 4022 passed, 0 failed, 149 skipped, 3 xfailed, rc=0,
  27.4-40.9 s wall (`time` real) over three runs**, measured 2026-09-20 in `xiaobizh_n18_cpu`
  ...
  captured before any pipe. The 4022 is **3956 ATOM tests + 66 `tests/compass/` tests**,
  stated as its parts because a single total cannot show which half moved (principle 7). The
  file count moves 128 → 129 and the test count 3956 → 4022 because this tree adds
  `tests/compass/test_cpu_gate_exclude.py`; the P0.2 readings above are the same suite
  without it. The other totals in circulation are the same suite under a different exclusion
  ...
  was this gate at `3afcb4880` with `tests/compass` at 49. The 49 → 66 step is mechanical:
  ...
  to 30.
- **Green is the bar, but green is not "exercised".** Of the 129 files handed, **22 collect
...
header included (188 test files, 129 CPU tier, 107 of them collecting, 47 candidates, 99
...
- *Coverage is credited only from a CPU-tier file that collects at least one test.* 22 of the
  129 CPU-tier files collect none. On this tree that probe **removes no path**: 30 triggers
