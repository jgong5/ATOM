...
> existing `--spec-decode-acceptance-*` flags, and the real rejection sampler runs.**

Three reasons this is better than anything Compass could add:

1. **It is ATOM's flag, not a Compass one** (`13` D80's non-flag audit). A real run can be
   driven with the identical argument, which is what makes a paired comparison a pairing.
2. **The rejection sampler still executes**, so acceptance affects the run through exactly
   the code path it affects a real run through — `SpecStats`, the bonus-token bookkeeping,
   the anchor gather, the scheduler's `max_tokens` interaction
   (`tests/test_scheduler_mtp_max_tokens.py`). Nothing is reimplemented.
3. **It is already rank-deterministic, deliberately.** The forced draw uses a dedicated
   device generator re-seeded from the step counter (`_SYNTHETIC_RNG_BASE_SEED`), because
   `sampled_tokens` is broadcast from rank 0 while `num_bonus_tokens` stays local and a
   per-rank `torch.rand` would desync the two — *"the anchor gather reads a rejected (−1)
   column → the draft model then embeds an invalid id (HSA out-of-bounds)"*. Compass gets
   reproducibility across ranks for free, which is exactly what `12` M-e asks for
   elsewhere.
...
**Where a real run exists, feed its measured per-position rates, not its mean.**

`synthetic_acceptance_rates` is already a **list** on the config, so this needs no new
mechanism — only that Compass populates it from measurement rather than from a scalar:
...
3. Report the three results of `08` D44 as usual — **plus** the acceptance agreement
   between the two sides, which under tier 1 should be near-exact and is therefore a
   *check that the feed worked*, not a result.
4. **If the simulated side's realised acceptance distribution differs materially from the
   input, something is wrong with the plumbing, not with the cost model.** That is a
...
| D83 | Acceptance is a **declared input**, supplied through ATOM's existing `--spec-decode-acceptance-*` flags. The real rejection sampler runs. Compass builds no acceptance model. | 2026-09-19 |
| D84 | Feed the **measured per-position distribution**, not the mean — the synthetic path's minimum-variance schedule matches an aggregate and not a schedule. Three input tiers with provenance; tier 3 is not acceptance evidence. | 2026-09-19 |
...
| D87 | A speculative throughput result is **conditional on its acceptance input**, and the artifact names the tier. Predicting acceptance itself is explicitly not attempted. | 2026-09-19 |
