...
| T73 | **Name a bootstrap that provably runs before the first `PhaseRunner` is constructed.** Successor to T10, opened 2026-09-20. **Not merely unproven — no such bootstrap exists today.** `discover_plugins()` imports no plugin module, so the Compass module is first imported inside `PhaseRunner._build_strategy` (`runner.py:488`), after `__init__` built the scheduler at `:191`. Phase 0 paces on the real clock, phase 1 gets the rebound class, and a smoke test passes (executed probe, 7/7). Candidates: a Compass plugin in a category resolved earlier (`system_controller.py:144/156`), or an explicit CLI/config hook. **Precondition of the seam, and W1.9's first deliverable** (`06` D34, `16` W1.9) |
