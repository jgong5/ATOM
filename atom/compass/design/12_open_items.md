...
| **T81** | Make the D18 capture *symbolic* on ATOM's real forward, or record that it cannot be. **Recorded: from the caller alone, it cannot.** The capture is concrete — 0 non-numeric shape entries of 13,047 at an honest TP2 — and the tracing mechanism is not the limitation: under the mode a GEMM and a softmax keep their free symbol with `shape_env.replacements` empty, and ATOM's own `CpuGpuBuffer`, unmodified, keeps it too when `copy_to_gpu` is handed the symbol instead of an integer. What blocks it sits upstream of that call. `prepare_decode` derives one count per staged buffer and uses it **twice** — to fill the buffer's **numpy** view (`var["slot_mapping"].np[:running_tokens]`) and then as the device copy's bound (`copy_to_gpu(num) for el, num in vars_used`) — and numpy takes `__index__` of whatever bound it is given, so a `SymInt` there resolves to its hint and the symbol becomes a constant **with no error and no warning**, before `copy_to_gpu` is reached. That also explains the earlier repair attempts' replacements (`{s64: 16384}`; `{s56: 2, s64: 16384}`): they are this substitution, not a tracing fault. A symbolic bound therefore needs `prepare_decode` to carry two numbers — a concrete one for the numpy fill, a symbolic one for the copy — which is a change to ATOM's serving path and is not made here. So the earlier "either a symbolic `CpuGpuBuffer` or a capture entry point below `prepare_inputs`" narrows to the second, and the first is measured unnecessary. Pinned by `tests/compass/test_capture_symbolic_shapes.py`, including a check that ATOM still shares the one bound, so the conclusion is retaken rather than inherited if that changes. Gates T5 alongside T52. |
