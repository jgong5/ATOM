...
| T70 | ~~Estimate~~ **Estimated 2026-09-22 by CA-7 (#50); what remains is a windowing policy, and the estimate says one is needed.** Grants scale with stages and microsecond lookahead, so the log is largest exactly where it is most wanted, and the measured numbers are worse than "large". Two disciplines, measured on the implementation at a 1 µs stage-to-stage floor, with a mean timeline record of **68 bytes**. *Every stage idle but one* (the affordable discipline): **3 grants and 170 bytes** for a 20 s stretch, at both a 1 ms and a 1 µs floor, and lookahead-independent. *Every stage holding work it cannot reach yet* — which is what a running pipeline is, since no resolve ever finds them all parked — each grant covers about one floor and the log grows with stages: **2 stages 1.0e6 grants/simulated second, 8 stages 7.0e6, 33 stages 3.2e7, 65 stages 6.4e7**. Against the 300 s modelled run `01` D3 sizes, that is **21 GB at PP2, 144 GB at PP8 and 1.3 TB at 65 participants** of timeline. So the log is affordable for a whole run only where the participants are mostly idle, and at PP degree > 1 it is a windowed artifact or it is nothing: the open part of this row is now **what the window is keyed on** — a virtual-time interval, a participant subset, or a ring buffer kept for the abort — and that belongs with whoever hits a causality failure at PP first. The always-written run summary is unaffected: it is read once at the end and its size does not depend on the grant count. |
