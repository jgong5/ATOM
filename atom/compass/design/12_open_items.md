...
| **T81** | Make the D18 capture *symbolic* on ATOM's real forward, or record that it cannot be. P0.4 measured the capture as **concrete**: 0 non-numeric shape entries out of 12,425 / 12,455 / 12,490 / 12,520 across the four committed records. Two repair attempts are measured and both fail for the load-bearing dimension: feeding D18's literal recipe (`from_tensor(static_shapes=False, symbolic_context=StatelessSymbolicContext(dynamic_sizes=...))`) to the runner's staging buffers and KV cache reaches **60 of 12,526** non-numeric entries, all of them on `prim.device` (38), `aten.slice` (13), `as_strided`, `fill_.Scalar`, `fill_.Tensor`, `reshape` — **zero** GEMM, attention or normalisation ops carry a symbol, and `shape_env.replacements = {s64: 16384}` means `capture()` refuses it anyway; injecting the SymInt as `ForwardMode.running_tokens`/`scheduled_tokens` after `decide` gives the same 60/12,526 with `{s56: 2, s64: 16384}`. The specialisation is ATOM's own input staging, not a tracing mistake: `model_runner.py:3252 forward → 2608 prepare_model → 2459 prepare_inputs → backends.py:661 build → gdn_attn.py:1451 / aiter_attention.py:1115 prepare_decode → atom/utils/__init__.py:725 CpuGpuBuffer.copy_to_gpu`, where `self.gpu[:n].copy_(self.cpu[:n])` slices a numpy-backed concrete CPU tensor by a Python `int`. So a symbolic capture of this model needs either a symbolic `CpuGpuBuffer` or a capture entry point below `prepare_inputs` — a W2.5 question, not a P0.4 one. Until then the artifact is what `02` D10.1 already names: *"The T5 fallback is the meta context plus concrete traces."* Gates T5 alongside T52. |
...
| `04` | D18's open issue *"Whether ATOM's real model classes trace cleanly under this mode at TP>1"* is answered in part and the answer is not "yes": the module tree builds and one forward dispatches (2,471 ops at TP1 decode; TP2 gets 2,477 before refusing at the sampler broadcast), but the trace that results is **concrete, not symbolic** — 0 non-numeric shape entries of 12,425–12,520 — so it does not carry D18's argument against route D. D18 should state the measurement and hand the symbolic question to W2.5 (T81) |
| `04` | D18's five `torch.cuda` stubs are not enough to import ATOM on this stack. Construction needs three more (`get_device_properties`, `current_device`, `get_device_capability` — the first is read at *import* by aiter's Triton attention configs) and running `ModelRunner.__init__` needs fourteen more, listed as BEYOND-D18 in `atom/compass/capture/fake_trace.py` and pinned by name in `tests/compass/test_capture_fake_trace.py`. `get_device_properties` and `mem_get_info` are *declared readings* in the `03` D14 sense and must be passed in, never read from a host |
...
| `04` | A TP>1 capture taken through `apply_simulated_tp` at one physical rank both **erases** and **fabricates**: 129 `all_reduce` per forward become the identity and appear nowhere in the inventory, while one `all_gather` becomes six real dispatched ops over a half-zeros tensor. Neither direction is visible in the operator list itself |
